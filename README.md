...
<details>
  <summary>💄 화장품 카테고리 프롬프트 예시 보기</summary>
  <br>
  
  * **시장 특성**: K-뷰티 글로벌 트렌드, 인플루언서 마케팅 영향도를 핵심 변수로 고려합니다.
  * **핵심 지표**: 단순 매출 외에 브랜드 인지도 상승률, 구매 전환율, 재구매율을 주요 지표로 설정합니다.
  * **분석 관점**: 연령대별 선호 브랜드, 계절성(여름/겨울), 트렌드 민감도를 중심으로 분석합니다.
  * **경쟁사 벤치마크**: 동급 브랜드와의 성과를 비교하고, 시장 점유율 변화를 추적합니다.
</details>
...
▼                                                         ▼
+-----------------+                                     +--------------------+
| Database (RDS)  |                                     | File Storage (S3)  |
| (MariaDB)       |                                     | (PDF, DOCX, XLSX)  |
+-----------------+                                     +--------------------+
...
<details>
  ...
  * **사용자 인증 및 권한 관리**: Supabase Auth 기반 소셜 로그인, JWT 세션, 역할 기반 접근 제어(RBAC)
  * **데이터 입력 및 파일 관리**: 스마트 카테고리, 동적 입력 필드, 다중 포맷(PDF, DOCX 등) 파일 업로드, AWS S3 보안 저장
  * **다차원 데이터 수집 및 전처리**: 시청자 행동/성과 데이터 통합, 이상치 및 결측값 처리, 데이터 정규화
  * **AI 기반 보고서 생성**: 프롬프트 오케스트레이션, 이중 AI 모델, 8단계 보고서 생성 파이프라인
  * **인터랙티브 대시보드 및 시각화**: 실시간 KPI 대시보드, 드릴다운 분석, 고급 시각화(D3.js, 히트맵 등)
  * **고급 내보내기 및 공유**: PDF, PowerPoint, Excel 등 다중 포맷 리포트 생성 및 실시간 웹 링크 공유
  * **API 및 시스템 통합**: RESTful API 설계, 외부 시스템(방송사, 소셜미디어 등) 연동
  
  ### 비기능 요구사항 (Non-functional Requirements)
  * **보안 및 컴플라이언스**: AES-256 저장 암호화, TLS 1.3 전송 암호화, 최소 권한 원칙 적용
  * **성능 및 확장성**: API 3초 이내 응답, 리포트 10분 이내 생성 등 성능 목표 정의
</details>
